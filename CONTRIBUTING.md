...
### Modules: headless data components

A component under the **Modules** tier renders a collection the caller owns — a
list, board, calendar, inbox or thread of records. It knows what an order or a
conversation _looks like_; it must not know where one comes from. That line is
what lets `SuperChatInbox` ship in a library while an app's inbox page cannot.

- **No data access.** No fetching, subscriptions, stores, routers, or app
  framework imports. `meteor/*`, `next/*`, `react-router*`, `@tanstack/react-query`
  and `@fortawesome/*` are blocked by `no-restricted-imports`; data arrives as
  `items` / `loading` / `error` props.
- **Mutations leave as callbacks** typed `(…args) => void | Promise<void>`. The
  module shows the pending state, awaits, and restores the previous state when the
  promise rejects — see `SuperChat`'s `onMessageSent`.
- **Navigation is a prop**: `onOpen(id)` and/or `getHref(id)`, never a router hook.
- **Generic over the item** — `<T>` plus accessor props or a config array, so an
  app's own types never have to match the library's.
- **Extension points follow `ChatComposer`**: `labels` for every user-facing
  string, `classNames` keyed by the component's `data-slot` names, and `render*`
  slots for domain content. Colour is a token _name_, never a class string.
- **Declare it** with `parameters.catalog.collection: true`. `pnpm catalog:check`
  then requires an `Empty`, `Loading` and `Error` story, because those three states
  are part of the module's API and no props table shows whether they are handled.
  Group `argTypes` under `Data` / `Callbacks` / `Slots`, and add a `Mobile` story.
...
| `parameters.catalog.entry` / `peers`    | Import path (`@mieweb/ui`, `@mieweb/ui/datavis`, `/esheet`, `/kerebron`, `/q`, `/templates`, `/ag-grid`) and optional peer packages. Rendered as the **Install / entry point** block.                                                                                                                                                                                  |
| `parameters.catalog.collection`         | `true` when the component renders a collection the caller owns — see [Modules: headless data components](#modules-headless-data-components). Requires an `Empty`, `Loading` and `Error` story.                                                                                                                                                           |
...
- **`@mieweb/ui/templates`** (`src/templates.ts`) is the Server-Component-safe
  entry for the Templates tier. Nothing reachable from it may use client-only
  React APIs (`useState`, `useEffect`, `createContext`, …) or `'use client'`;
  `src/templates/server-safe.test.ts` enforces this.
...
| [Templates](src/templates/MAINTAINERS.md)                                    | Server-Component-safe `@mieweb/ui/templates` entry (test + CI guard); JSON-only block data; `components`/`trackingId` threading; tone contrast tuned against axe; marquee CSS side effect; TW3 safelist block |
| [Views](src/views/MAINTAINERS.md)                                            | Shared accessor contract across six layouts; wall-date handling via `toDateTime`; accent tokens carry no text colour; ViewSet-owned shared props; board keyboard move is not dnd-kit's                          |
