...
- i18n: all strings are props with English defaults (\`placeholder\`, \`inputLabel\`, \`addMenuLabel\`, \`attachFilesLabel\`, \`micLabel\`, \`sendLabel\`, \`stopLabel\`, \`agentSelectorLabel\`, \`readOnlyMessage\`); pass translated values. RTL: uses logical properties (\`ms-auto\`, \`pe-*\`) throughout.
- Accessibility: every icon button has an \`aria-label\`; the \`+\` and agent menus are \`Dropdown\`s with full keyboard support; the character counter is \`aria-live="polite"\`. The textarea is labelled via \`inputLabel\`.
- Types exported: \`ChatComposerProps\`, \`ChatComposerHandle\`, \`ChatComposerMenuItem\`, \`ChatComposerAgentOption\`; the send payload reuses \`NewMessage\` from the Messaging module. Entry \`@mieweb/ui\`.`,
      },
    },
    catalog: {
      entry: '@mieweb/ui',
