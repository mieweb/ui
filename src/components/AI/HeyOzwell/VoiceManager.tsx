...
export function VoiceManager({ logoSrc }: VoiceManagerProps) {
  ...
  const [confirm, setConfirm] = React.useState<
    { type: 'remove'; id: string; label: string } | { type: 'clearAll' } | null
  >(null);
  ...
  const startFresh = () =>
    setInSetup({ mode: 'enroll', voiceId: 'you', label: 'You' });
  const startAdd = () => {
  ...
  return (
    <div className="bg-background text-foreground flex min-h-full flex-col items-center justify-center p-6 text-center">
      ...
      {!sv.ready && !sv.error && (
        <div className="text-muted-foreground mt-4 font-mono text-[13px]">
          Loading…
        </div>
      )}
      {sv.error && (
        <div className="text-destructive mt-4 text-[13px]">
      ...
      {sv.ready && voices.length === 0 && (
        <>
          <p className="text-muted-foreground mt-2.5 max-w-[440px] leading-relaxed">
            Set up your voice so Ozwell only responds to you. You can add an
            assistant or extra conditions afterward.
          </p>
      ...
      {sv.ready && voices.length > 0 && (
        <>
          <p className="text-muted-foreground mt-2 text-[14.5px]">
            Voices Ozwell will respond to. Add an assistant or another
                ...
                {editingId === v.id ? (
                  ...
                  <>
                    ...
                    <Button
                      ...
                      onClick={() =>
                        setConfirm({ type: 'remove', id: v.id, label: v.label })
                      }
                    >
                      Remove
                    </Button>
          ...
          <div className="mt-[18px] flex w-full max-w-[460px] gap-2">
            ...
            />
            <Button size="sm" className={ozBtn} onClick={startAdd}>
              Add a voice
            </Button>
          </div>
