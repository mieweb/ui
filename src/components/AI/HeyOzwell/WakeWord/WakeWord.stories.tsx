...
const LABELS: Record<string, string> = {
  'hey-ozwell': 'hey ozwell',
  "ozwell-i'm-done": "ozwell I'm done",
};
...
function WakeWordDemo({
  ...
  const thresholds = React.useMemo(
    () => ({
      'hey-ozwell': heyOzwellThreshold,
      "ozwell-i'm-done": doneThreshold,
    }),
  ...
  const st = useWakeWord({
    ...
    onWake: (name) => {
      setLog((l) =>
        [{ name, t: new Date().toLocaleTimeString() }, ...l].slice(0, 6)
      );
      setFlash(name);
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <span
          style={{
            ...
            background: dotColor,
            animation:
              status === 'listening' ? 'oz-pulse 1.4s infinite' : 'none',
          }}
        />
      ...
      <p style={{ color: '#64748b', margin: '6px 0 4px', fontSize: 13.5 }}>
        Say <b style={{ color: '#1f2733' }}>“hey ozwell”</b> or{' '}
        <b style={{ color: '#1f2733' }}>“ozwell I’m done”</b>. Detection runs
        fully on-device — audio never leaves the page.
      </p>
...
export const Listener: StoryObj<WakeArgs> = {
  args: {
    heyOzwellThreshold: 0.8,
    doneThreshold: 0.5,
    speechVadThreshold: 0.05,
    silenceVadThreshold: 0.03,
  },
  argTypes: {
    heyOzwellThreshold: {
      ...
      description:
        'Fire gate for hey-ozwell. Higher = fewer false fires but more misses.',
    },
    doneThreshold: {
    ...
    speechVadThreshold: {
      ...
      control: { type: 'range', min: 0, max: 1, step: 0.01 },
      description:
        'Speech-detect gate — higher needs louder/clearer speech before the wake models run.',
    },
    silenceVadThreshold: {
      name: 'VAD silence threshold',
      control: { type: 'range', min: 0, max: 1, step: 0.01 },
      description:
        'Silence gate — when speech probability drops below this it’s treated as quiet.',
    },
  },
  ...
  return (
    ...
    >
      ...
      <p style={{ color: '#64748b', fontSize: 13.5, marginTop: 0 }}>
        Diagnostic only — is the mic even captured in Storybook?{' '}
        <b>Talk and this bar should jump.</b> It reads the mic directly and
        bypasses the detector. If it moves but the Listener’s “speech” bar
        doesn’t, the detector’s audio path is the problem. If this stays flat
        too, the iframe isn’t getting mic audio.
      </p>
      {err ? (
        <div style={{ color: '#dc2626', font: '12px monospace' }}>
          error: {err}
        </div>
      ) : (
        <>
          ...
          >
            raw peak: {level.toFixed(3)}{' '}
            {level > 0.05 ? '✅ capturing' : '— quiet'}
          </div>
        </>
      )}
    </div>
