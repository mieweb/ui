...
export function generateBrandCSS(brand: BrandConfig): string {
  ...
  // Optional brand accent — components fall back to the primary scale when
  // these variables are absent, so only emit them when the brand defines one.
  const accentBlock = colors.accent
    ? `

  /* Brand Accent */
  --mieweb-accent: ${colors.accent.DEFAULT};${colors.accent.light ? `\n  --mieweb-accent-light: ${colors.accent.light};` : ''}${colors.accent.dark ? `\n  --mieweb-accent-dark: ${colors.accent.dark};` : ''}`
    : '';
